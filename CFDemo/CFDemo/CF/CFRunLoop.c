...
static void __CFRunLoopDeallocate(CFTypeRef cf) {
    ...
    // 当前线程的运行循环与要销毁的一致
    if (_CFRunLoopGet0b(pthread_main_thread_np()) == cf) HALT;//执行汇编int3指令
    ...
    /* We try to keep the run loop in a valid state as long as possible,
       ...
       three lines.
     我们尽量保持运行循环处于有效状态，因为事件源可能有非保留的运行循环引用。另一个原因是我们不想为回调原因锁定运行循环，如果可以的话没有这个，就离开了。 我们从移除事件源开始，因为它们最有可能在其“取消”期间回调到运行循环中。 共模式项目将通过以下三行间接从模式中删除。*/
...
/**
 * 通过给定的线程pthread_t对象创建CFRunLoop
 * @param t 某线程
 * return 返回CFRunLoop
 */
static CFRunLoopRef __CFRunLoopCreate(pthread_t t) {
    CFRunLoopRef loop = NULL;
    CFRunLoopModeRef rlm;// CFRunLoop运行模式
...
// 只应由Foundation框架调用，t == 0是始终有效的“主线程”的同义词
CF_EXPORT CFRunLoopRef _CFRunLoopGet0(pthread_t t) {
    // t==0 is a synonym(同义词) for "main thread(主线程)" that always works
    if (pthread_equal(t, kNilPthreadT)) {// pthread_equal(t, (pthread_t)0)
        t = pthread_main_thread_np();
    }
    __CFLock(&loopsLock);
    // static CFMutableDictionaryRef __CFRunLoops = NULL;
    if (!__CFRunLoops) {// 字典为空,以主线程为键值，mainLoop为值，存储在字典中
...
CFRunLoopRef CFRunLoopGetMain(void) {
    ...
    if (__main == NULL) {
        __main = _CFRunLoopGet0(pthread_main_thread_np()); // no CAS needed "Check And act”模式
    }
    return __main;
}
...
// 在按钮调用栈中可以看到__CFRUNLOOP_IS_CALLING_OUT_TO_A_SOURCE0_PERFORM_FUNCTION__这个东东
static Boolean __CFRunLoopDoSources0(CFRunLoopRef rl, CFRunLoopModeRef rlm, Boolean stopAfterHandle) {	/* DOES CALLOUT */
    ...
    if (NULL != rlm->_sources0 && CFSetGetCount(rlm->_sources0) > 0) {
...
/**
 * @param rl                          传入runloop对象，实际传入的都是CFRunLoopGetCurrent
 * @param modeName                    传入当前要运行的mode名称
 * @param seconds                     Runloop的超时时间
 * @param returnAfterSourceHandled    主要__CFRunLoopRun会用到，后面说
 */ 
signed int CFRunLoopRunSpecific(CFRunLoopRef rl, CFStringRef modeName, CFTimeInterval seconds, Boolean returnAfterSourceHandled) {     /* DOES CALLOUT */
    CHECK_FOR_FORK();
...
void CFRunLoopAddSource(CFRunLoopRef rl, CFRunLoopSourceRef rls, CFStringRef modeName) {	/* DOES CALLOUT */
    ...
    if (doVer0Callout) {
        ...
        // 尽管它对源代码有一些保护，但我们别无选择，只有在解锁运行循环和模式锁定之后才能做到这一点，以避免源代码想要获取已锁在另一个线程中的死锁，这个线程本身正在等待 运行循环/模式锁定
...
void CFRunLoopSourceSignal(CFRunLoopSourceRef rls) {
    ...
    __CFRunLoopSourceLock(rls);
    if (__CFIsValid(rls)) {
        __CFRunLoopSourceSetSignaled(rls);// 标记为待执行
    }
    __CFRunLoopSourceUnlock(rls);
}
