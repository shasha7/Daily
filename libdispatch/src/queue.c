...
_dispatch_get_root_queue(dispatch_qos_t qos, bool overcommit)
{
	if (unlikely(qos == DISPATCH_QOS_UNSPECIFIED || qos > DISPATCH_QOS_MAX)) {
		DISPATCH_CLIENT_CRASH(qos, "损坏 priority");
	}
...
{
	// _dispatch_async_f(dq, ctxt, func, 0, 0);等效于下面的代码
	{
		// tls技术,线程局部存储技术
		dispatch_continuation_t dc = _dispatch_continuation_alloc_cacheonly();
		...
		// _dispatch_continuation_init_f(dc, dq, ctxt, func, pp, flags, dc_flags);等效于下面的代码
		{
			// 简单赋值
			dc->dc_flags = dc_flags;
			dc->dc_func = func;
		...
		// _dispatch_continuation_async2(dq, dc, false);等效于下面的代码
		{
			if (fastpath(barrier || !DISPATCH_QUEUE_USES_REDIRECTION(dq->dq_width))) {
				return _dispatch_continuation_push(dq, dc);
			}
 ...
 * for objects with a more complex state machine (sources or mach channels)
 对于队列，我们可以作弊并内嵌解锁代码，这对于具有更复杂状态机（源或mach通道）的对象是无效的，
 */
DISPATCH_NOINLINE
static void
...
dispatch_barrier_sync_f(dispatch_queue_t dq, void *ctxt,
		dispatch_function_t func)
{
	dispatch_tid tid = (dispatch_tid)pthread_mach_thread_np(_dispatch_thread_self());

	// 更正确的做法是将刚刚获得barrier锁的线程的优先级合并到队列状态中
	// 然而，这对快速路径来说太昂贵了，所以不要这样做。所选择的折衷是如果在较低优先级线程上的入队与该快速路径竞争，则该线程可能会收到无用的覆盖
	// 绑定到非分派线程的全局并发队列和队列始终属于慢速情况，请参阅DISPATCH_ROOT_QUEUE_STATE_INIT_VALUE
	...
	if (unlikely(!_dispatch_queue_try_acquire_barrier_sync_and_suspend(dq, tid, 0))) {
...
dispatch_sync_f(dispatch_queue_t dq, void *ctxt, dispatch_function_t func)
{
	...
	// 串行队列
	// 必须要等待前面的任务执行完成之后才能执行该任务，因此会调用dispatch_barrier_sync_f
	...
	// Global concurrent queues and queues bound to non-dispatch threads
	// always fall into the slow case, see DISPATCH_ROOT_QUEUE_STATE_INIT_VALUE
	// 绑定到非分派线程的全局并发队列和全局并发队列队列始终属于slow情况，请参阅DISPATCH_ROOT_QUEUE_STATE_INIT_VALUE
