...
{
	// _dispatch_thread_frame_save_state(dtf);等效以下代码
	{
	...
	// _dispatch_thread_setspecific_pair(dispatch_queue_key, dq, dispatch_frame_key, dtf);等效一下代码
...
{
	// 以__PTK_LIBDISPATCH_KEY2为键
	dispatch_continuation_t dc = (dispatch_continuation_t)
			_dispatch_thread_getspecific(dispatch_cache_key);
	
	// #define likely(x) __builtin_expect(!!(x), 1)内置函数用于优化
	// long __builtin_expect (long EXP, long C)
	// 它的返回值就是整个函数的返回值，参数 C 代表预计的值，表示程序员知道EXP的值很可能就是 C。这里就是指线程局部存储的值很可能存在
	if (likely(dc)) {
		// 去链表/线程池的下一个存储到tls
