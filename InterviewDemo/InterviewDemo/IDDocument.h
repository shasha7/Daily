...
/*
 Q:@property (strong, nonatomic) UIWindow *window;
 这里不禁要问@到底是什么东西？？
...
/*
 ...
 NSObject+NSKeyValueCoding类扩展
 // Dog *dog = [[Dog alloc] init];
 // [dog setValue:nil forKey:@"age"];
 // NSString *name = [dog valueForKey:@"_name"];
 // NSLog(@"%@", name);
 //valueForKey：会自动将值类型封装成对象，但是setValue：forKey：却不行。你必须手动将值类型转换成NSNumber或者NSValue类型，才能传递过去。
 //KVC和字典
